...
class MindfulPage(tk.Frame):
    ...
    def create_widgets(self):
        ...
        self.content_var.set('book')  # set default value
        ...
        # Search button
        search_button = tk.Button(self, text="Search", command=self.search_content)
    ...
    def search_content(self):
        ...
        results = self.get_content_from_db(content_type, search_tags)
    ...
    def get_content_from_db(self, content_type, search_tags):
        # Connect to the SQLite database
        conn = sqlite3.connect('project.db')
        cursor = conn.cursor()

        # Fetch content based on the content type and search tags
        search_query = f"%{search_tags}%"
        cursor.execute("SELECT title, url FROM mindful_space WHERE contentType=? AND tags LIKE ?",
